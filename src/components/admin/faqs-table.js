...
export default function FaqsTable({ initialItems, canDelete }) {
        ...
        <tbody>
          {items.map((f) => (
            <tr key={f._id} className="border-t border-slate-100">
              <td className="px-4 py-3 text-slate-600">{f.order ?? 0}</td>
              <td className="px-4 py-3 font-medium text-slate-900">{f.question}</td>
      ...
      {confirmId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-sm rounded-lg bg-white p-6 shadow-xl">
            <h2 className="text-base font-semibold text-slate-900">Delete this FAQ?</h2>
